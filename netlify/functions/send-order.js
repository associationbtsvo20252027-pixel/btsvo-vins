...
exports.handler = async function(event) {
  ...
  const itemsHtml = (items||[]).map(i => {
    const details = [i.appellation, i.exploitation, i.couleur, i.millesime ? 'Mill. '+i.millesime : ''].filter(Boolean).join(' · ');
    return '<tr>'
      + '<td style="padding:8px 12px;border-bottom:1px solid #f0e8df"><span style="font-weight:600;color:#1A1008">' + (i.nom||'—') + '</span>'
  ...
  const msgRow = message
    ? '<tr><td style="padding:4px 0;color:#6B5B4E;vertical-align:top">Message</td><td style="padding:4px 0;color:#1A1008;font-style:italic">' + message + '</td></tr>'
    : '';
  ...
  const htmlAdmin = '<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"></head>'
    ...
    + '<h1 style="color:#fff;font-size:22px;font-weight:300;margin:0;font-style:italic">Nouvelle commande recue</h1>'
    + '</div>'
    ...
    + '<tr><td style="padding:4px 0;color:#6B5B4E">Telephone</td><td style="padding:4px 0;color:#1A1008">' + (tel||'—') + '</td></tr>'
    + '<tr><td style="padding:4px 0;color:#6B5B4E">Retrait</td><td style="padding:4px 0;font-weight:600;color:#1A1008">' + (ville||'—') + '</td></tr>'
    + '<tr><td style="padding:4px 0;color:#6B5B4E">Paiement</td><td style="padding:4px 0;color:#1A1008">' + (paiement||'Non precis') + '</td></tr>'
    + '<tr><td style="padding:4px 0;color:#6B5B4E">Date</td><td style="padding:4px 0;color:#1A1008">' + dateStr + ' a ' + timeStr + '</td></tr>'
    + msgRow
    + '</table></div>'
    + '<p style="font-size:11px;font-weight:600;color:#6B5B4E;letter-spacing:.1em;text-transform:uppercase;margin:0 0 10px">Detail de la commande</p>'
    ...
    + '<th style="padding:8px 12px;text-align:center;font-size:11px;color:#6B5B4E;font-weight:500;text-transform:uppercase">Qte</th>'
    ...
    + 'Recontacter le client pour confirmer la commande.<br>'
    + '<strong>Email :</strong> ' + email + ' — <strong>Tel :</strong> ' + (tel||'—')
    + '</p></div>'
    + '<div style="text-align:center;padding:16px;border-radius:0 0 12px 12px;background:#EDE6D9">'
    + '<p style="color:#6B5B4E;font-size:11px;margin:0">Association BTS VO — Millesime 2025-2027</p>'
    + '</div></div></body></html>';
